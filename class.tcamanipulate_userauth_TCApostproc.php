    ...
    */
    function processTCAtitles(&$params, &$reference)
    {
        ...
        if (is_object($GLOBALS['BE_USER']) && $TCA) {
                    ...
                    foreach ($editconf as $table => $uidvalue) {
                        if (is_array($uidvalue)) {
                            if ((current($uidvalue) == 'new' AND key($uidvalue) > 0) OR $table == 'pages') {
                                ...
                                $recordArr = \TYPO3\CMS\Backend\Utility\BackendUtility::getRecord($table,
                                    intval(abs(key($uidvalue))), 'pid');
            ...
            if (intval($pageid)) {
                $includeLibs = \TYPO3\CMS\Backend\Utility\BackendUtility::getModTSconfig($pageid,
                ...
                $modTSconfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getModTSconfig($pageid,
                    'TCA'); //Gets the TSconfig
